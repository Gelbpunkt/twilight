use crate::{
    traits::CacheableChannel, CacheableModels, InMemoryCache, ResourceType, UpdateCache,
    UpdateResult,
};
use twilight_model::{
    channel::Channel,
...
impl<CacheModels: CacheableModels> InMemoryCache<CacheModels> {
    ...
    pub(crate) fn cache_channel(&self, channel: Channel) -> Option<CacheModels::Channel> {
    ...
    ) -> Option<CacheModels::Channel> {
        let (_, channel) = self.channels.remove(&channel_id)?;
        if let Some(guild_id) = channel.guild_id() {
...
impl<CacheModels: CacheableModels> UpdateCache<CacheModels> for ChannelCreate {
    type Previous = Option<CacheModels::Channel>;
    ...
    fn update(&self, cache: &InMemoryCache<CacheModels>) -> UpdateResult<Self::Previous> {
        ...
        UpdateResult::Processed(cache.cache_channel(self.0.clone()))
    }
}
...
impl<CacheModels: CacheableModels> UpdateCache<CacheModels> for ChannelDelete {
    ...
    fn update(&self, cache: &InMemoryCache<CacheModels>) -> UpdateResult<Self::Previous> {
        ...
        UpdateResult::Processed(cache.delete_channel(self.0.id))
    }
}

impl<CacheModels: CacheableModels> UpdateCache<CacheModels> for ChannelPinsUpdate {
    type Previous = Option<Timestamp>;
    ...
    fn update(&self, cache: &InMemoryCache<CacheModels>) -> UpdateResult<Self::Previous> {
        ...
        if let Some(mut channel) = cache.channels.get_mut(&self.channel_id) {
            UpdateResult::Processed(channel.set_last_pin_timestamp(self.last_pin_timestamp))
        } else {
